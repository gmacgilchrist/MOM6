!> Regrid columns for the arbitrary scalar coordinate
!!! Note that this coordinate is for diagnostic purposes only !!!
module coord_scalar
...
subroutine init_coord_scalar(CS, nk, ref_pressure, target_density, needs_sorting, interp_CS)
  ...
  if (associated(CS)) call MOM_error(FATAL, "init_coord_scalar: CS already associated!")
  allocate(CS)
...
end subroutine build_scalar_column_iteratively
